...
def setup_llama_index(OCTOAI_API_KEY):
    print("Debug: Inside setup_llama_index")
...
def create_react_agent(uploaded_files, OCTOAI_API_KEY, llm):
    print("Debug: Inside create_react_agent")
    if 'storage_context' not in st.session_state:
        print("Debug: Initializing storage_context")
        st.session_state.storage_context = StorageContext.from_defaults()

    try:
        print("Debug: Attempting to load index from storage")
        storage_context = StorageContext.from_defaults(persist_dir="./storage")
        index = load_index_from_storage(storage_context)
        index_loaded = True
        print("Debug: Index loaded successfully")
    except Exception as e:
        print(f"Debug: Failed to load index: {str(e)}")
        index_loaded = False

    if uploaded_files and not index_loaded:
        print("Debug: Processing uploaded files")
        all_docs = []
        ...
        index_loaded = True
        print("Debug: New index created and loaded")
    ...
    if index_loaded:
        ...
        print("Debug: Creating ReActAgent")
        return ReActAgent.from_tools([query_engine_tool], llm=llm, verbose=True, max_turns=10)

    print("Debug: No index loaded, returning None")
    return None
